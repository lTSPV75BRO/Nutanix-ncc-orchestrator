...
func writeScheduleRunnerScript(taskName, runCmd, logPath string) (string, error) {
	...
	content := strings.Join([]string{
		...
		// NCC_RUN_SOURCE tags the run as scheduled so the dashboard can label it
		// and the api-server can surface it as a scheduled run in Active Runs.
		"NCC_RUN_SOURCE=scheduled exec " + runCmd + " >> " + shellQuotePOSIX(logPath) + " 2>&1",
		"",
	}, "\n")
