...
**`verify --online`.** `ncc-orchestrator verify` can now fetch the matching release's `checksums.txt` from GitHub and report **MATCH / MISMATCH / NOT FOUND** for this platform's binary, exiting non-zero on a mismatch (handy in CI / health checks). It verifies against the binary's stamped version (`--release` / `--repo` override), skips locally-modified builds, and reuses `update`'s hardened client and checksum parser; the default `verify` stays offline. This authenticates a corrupted or MITM'd download and pins the version — signed checksums / attestations for full provenance are future work.
