...
- **Optional code-signing hooks** in `binaryGO.txt`: macOS Developer ID + notarization (`codesign` + `xcrun notarytool`), Windows Authenticode (`osslsigncode`), Linux GPG detached signatures. No-op unless the matching env vars are set.
