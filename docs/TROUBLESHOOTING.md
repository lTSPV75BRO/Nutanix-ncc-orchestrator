...
- **Expected:** Ingress terminates TLS. `GET /api/v1/settings/tls` reports
  `managed_by=ingress`. PUT/DELETE/generate return `409`. Update secret
  `ncc-v2-ui-tls` or enable cert-manager on the Ingress.
